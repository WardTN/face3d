...
import com.wardtn.facemodel.collision.Octree;
import com.wardtn.facemodel.drawer.RendererFactory;
import com.wardtn.facemodel.services.collada.entities.MeshData;
import com.wardtn.facemodel.util.android.AndroidUtils;
...
public class Object3DData {
    ...
    private String tetxurePath; //纹理JPG路径

    public void setTetxurePath(String tetxurePath) {
        this.tetxurePath = tetxurePath;
    }

    public String getTetxurePath() {
        return tetxurePath;
    }
    ...
    private String name;
    /**
     * Whether to draw object using indices or not  是否使用索引绘制对象
     */
    ...
    public byte[] getOverlayTextureData() {
        return getMaterial().getOverlayData();
    }
    ...
    public void setOverLayTextureData(byte[] textureData) {
        Log.i("Object3DData", "New texture: " + textureData.length + " (bytes)");
        this.getMaterial().setOverlayData(textureData);
        if (this.getElements() != null && this.getElements().size() == 1) {
            // TODO: let user pick object and/or element to update texture
            // as for now, let's just update 1st element
            for (int i = 0; i < 1; i++) {
                if (getElements().get(i).getMaterial() == null) continue;
                if (getElements().get(i).getMaterial().getOverlayData() == null) continue;
                this.getElements().get(i).getMaterial().setOverlayData(textureData);
                Log.i("Object3DData", "New texture for element (" + i + "): " + getElements().get(i).getMaterial());
            }
        }
    }
