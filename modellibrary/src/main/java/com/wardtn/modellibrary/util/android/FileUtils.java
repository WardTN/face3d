...
public class FileUtils {
    ...
    public static String get3DDownloadPath() {
        return Constant.PATH_Folder_3D;
    }

    public static String getRenderImg(int light) {
        return Constant.SOLEX + ".jpg";
    }
    ...
    public static void createChooserDialog(Context context, String title, CharSequence message, File folder, String fileRegex, Callback callback) {
    ...
    private static File addParentFile(File folder, List<File> fileList) {
        ...
        if (folder != null) {
            if (!(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).equals(folder) || Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).equals(folder) || Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM).equals(folder) || (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS).equals(folder)) || (isExternalStorageReadable() && Environment.getExternalStorageDirectory().equals(folder)))) {
    ...
    private static List<File> listFiles(File folder, String fileRegex) {
        ...
        if (folder == null) {
            ...
            try {
                File[] children = folder.listFiles(file -> !file.isHidden() && file.exists() && (file.isDirectory() || file.getName().matches(fileRegex)));
                if (children != null) {
