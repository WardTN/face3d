...
import com.wardtn.modellibrary.model.CubeMap;
...
public final class GLUtil {
    ...
    public static int loadTexture0(final byte[] textureData) {
        ByteArrayInputStream textureIs = new ByteArrayInputStream(textureData);
        return loadTexture(textureIs);
    }

    public static int loadTexture1(final byte[] textureData) {
        ByteArrayInputStream textureIs = new ByteArrayInputStream(textureData);
        return loadTexture1(textureIs);
    }


    private static int[] textureHandle1;
    private static int[] textureHandle2;

    public static int loadTexture(final InputStream is) {
        Log.v("GLUtil", "Loading texture from stream...");

        textureHandle1 = new int[1];

        GLES20.glGenTextures(1, textureHandle1, 0);
        GLUtil.checkGlError("glGenTextures");
        if (textureHandle1[0] == 0) {
            throw new RuntimeException("Error loading texture.");
        }

        Log.v("GLUtil", "Handler: " + textureHandle1[0]);

        final Bitmap bitmap = loadBitmap(is);

        // Bind to the texture in OpenGL
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureHandle1[0]);
        ...
        return textureHandle1[0];
    }

    public static int loadTexture1(final InputStream is) {
        Log.v("GLUtil", "Loading texture from stream...");

        textureHandle2 = new int[1];

        GLES20.glGenTextures(1, textureHandle2, 0);
        GLUtil.checkGlError("glGenTextures");
        if (textureHandle2[0] == 0) {
            throw new RuntimeException("Error loading texture.");
        }

        Log.v("GLUtil", "Handler: " + textureHandle2[0]);

        final Bitmap bitmap = loadBitmap(is);

        // Bind to the texture in OpenGL
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureHandle2[0]);
        GLUtil.checkGlError("glBindTexture");
        GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, bitmap, 0);
        GLUtil.checkGlError("texImage2D");
        bitmap.recycle();
    ...
    public static void deleteTexture() {
        GLES20.glDeleteTextures(1, textureHandle1, 0);
    }

    public static void deleteTexture1() {
        GLES20.glDeleteTextures(1, textureHandle2, 0);
    }
